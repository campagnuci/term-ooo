// src/naruto/useNarutoGame.ts
// Estado do Shinobi: palpites do dia (persistidos por dateKey), vitória e
...
const STATE_PREFIX = 'shinobi:state'
const STATS_KEY = 'shinobi:stats'
...
const EMPTY_STATS: ShinobiStats = { wins: 0, currentStreak: 0, maxStreak: 0, lastWinDay: null }
...
function loadStats(): ShinobiStats {
  try {
...
export function useNarutoGame() {
  ...
  const [stats, setStats] = useState<ShinobiStats>(loadStats)

  // Mais recente primeiro, como no Narutodle
  const guesses: GuessComparison[] = useMemo(
    () =>
      guessIds
        .map(id => getCharacterById(id))
        .filter((c): c is NarutoCharacter => c != null)
