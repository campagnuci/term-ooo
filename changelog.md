...
- Modal de vitória do Jogo da Memória: pontuação numérica 0–100 por eficiência de jogadas (pares ÷ jogadas × 100; jogo perfeito = 100) — as estrelas agora derivam da pontuação com cortes equivalentes aos originais (≥ 62,5 → 3★, ≥ ~43,5 → 2★); helpers `computeScore`/`starsForScore` exportados para a futura pontuação entre jogos — e novo botão "Início" para voltar ao hub
