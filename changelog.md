...
### Changed
- Tema do Jogo da Memória alinhado à paleta da marca: acentos pistache/eucalipto e fundos black glaze no lugar do dourado/azul do protótipo (coral mantido para erros/sequências)
- Modal de vitória do Jogo da Memória: pontuação numérica da partida (0–1000) = **700 × precisão + 300 × bônus de tempo**, com detalhamento "Precisão · Tempo" exibido sob o valor. Precisão = pares ÷ jogadas (jogo perfeito = 700); bônus de tempo integral até o tempo-alvo da dificuldade (`parSeconds`: 40s/65s/100s) e decaindo pela metade a cada tempo-alvo adicional. **As estrelas continuam derivando só da precisão**, com cortes equivalentes aos originais (eficiência ≥ 62,5% → 3★, ≥ ~43,5% → 2★) — o tempo nunca muda a avaliação por estrelas. Helpers `computeScore`/`starsForScore`/`computeTimeFactor`/`computeMatchScore`/`SCORE_WEIGHTS` exportados para a futura pontuação entre jogos — e novo botão "Início" para voltar ao hub
